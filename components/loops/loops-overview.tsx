...
export default function LoopsOverview({ data }: { data: LoopsOverviewData }) {
  if (!data.ok) {
    return (
      <div className="p-8">
        <div className="text-red-400">Failed to load loops overview</div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-zinc-950 via-zinc-900 to-zinc-950 text-white p-8">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-3xl font-bold mb-2">Loops - Stress Removal</h1>
        ...
        {data.items && data.items.length > 0 && (
          <div className="mt-8">
            <h2 className="text-xl font-semibold mb-4">Active Loops</h2>
            <div className="space-y-2">
              {data.items.slice(0, 10).map((item, idx) => (
                <div key={idx} className="p-4 bg-zinc-800/50 rounded border border-zinc-700">
                  <div className="font-medium">{item.name || JSON.stringify(item).slice(0, 100)}</div>
                </div>
              ))}
            </div>
          </div>
