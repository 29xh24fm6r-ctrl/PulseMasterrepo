...
export default function TimeOverview({ data }: { data: TimeOverviewData }) {
  if (!data.ok) {
    return (
      <div className="p-8">
        <div className="text-red-400">Failed to load time overview</div>
      </div>
    );
  }
            ...
            <div className="space-y-2">
              {data.items.slice(0, 10).map((item, idx) => (
                <div key={idx} className="p-4 bg-zinc-800/50 rounded border border-zinc-700">
                  ...
                  {item.start_time && (
                    <div className="text-sm text-gray-400">{new Date(item.start_time).toLocaleTimeString()}</div>
                  )}
                </div>
              ))}
            </div>
