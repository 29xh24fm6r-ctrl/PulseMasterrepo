...
import { RouteIdentityBadge } from "@/app/(pulse)/_components/RouteIdentityBadge";
...
export default function WorkspaceOverview({ data }: { data: WorkspaceOverviewData }) {
  if (!data.ok) {
    return (
      <div className="p-8">
        <div className="text-red-400">Failed to load workspace overview</div>
      </div>
    );
  }
  ...
  return (
    <div className="text-white">
      <RouteIdentityBadge id="WORKSPACE_PAGE" />
      <div>
        <h1 className="text-3xl font-bold mb-2">Workspace - Active Self</h1>
            ...
            <div className="space-y-2">
              {data.items.slice(0, 10).map((item, idx) => (
                <div key={idx} className="p-4 bg-zinc-800/50 rounded border border-zinc-700">
                  <div className="font-medium">{item.title || item.name || JSON.stringify(item).slice(0, 100)}</div>
                </div>
              ))}
            </div>
          </div>
