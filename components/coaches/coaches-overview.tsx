...
export default function CoachesOverview({ data }: { data: CoachesOverviewData }) {
  if (!data.ok) {
    return (
      <div className="p-8">
        <div className="text-red-400">Failed to load coaches overview</div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-zinc-950 via-zinc-900 to-zinc-950 text-white p-8">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-3xl font-bold mb-2">Coaches - Specialized Lenses</h1>
          ...
          <div className="mt-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {data.items.map((item, idx) => (
                <Link
                  key={idx}
                  href={item.href || `/coaches/${item.id || item.type || idx}`}
                  className="p-4 bg-zinc-800/50 rounded border border-zinc-700 hover:border-purple-500/50 transition-colors"
                >
                  <div className="font-medium">{item.name || item.id || item.type || "Coach"}</div>
                </Link>
              ))}
            </div>
          </div>
