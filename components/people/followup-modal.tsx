...
export default function FollowupModal({ personId, personName, onClose, onSuccess }: FollowupModalProps) {
  const [channel, setChannel] = useState<"email" | "call" | "text" | "sms">("email");
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-zinc-800 rounded-lg border border-zinc-700 w-full max-w-md">
        <div className="flex items-center justify-between p-4 border-b border-zinc-700">
          <h2 className="text-lg font-semibold">Schedule Follow-up with {personName}</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-4">
          ...
          <div>
            ...
            <select
              value={channel}
              onChange={(e) => setChannel(e.target.value as "email" | "call" | "text" | "sms")}
          ...
          <div>
            ...
            <textarea
              ...
              rows={4}
              className="w-full px-3 py-2 bg-zinc-900 border border-zinc-700 rounded text-white placeholder-gray-500 focus:outline-none focus:border-green-500 resize-none"
              maxLength={5000}
            />
          </div>
