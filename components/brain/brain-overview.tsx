...
export default function BrainOverview({ data }: { data: BrainOverviewData }) {
  if (!data.ok) {
    return (
      <div className="p-8">
        <div className="text-red-400">Failed to load brain overview</div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-zinc-950 via-zinc-900 to-zinc-950 text-white p-8">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-3xl font-bold mb-2">Brain - Memory & Intel</h1>
            ...
            <div className="space-y-2">
              {data.items.slice(0, 10).map((item, idx) => (
                <div key={idx} className="p-4 bg-zinc-800/50 rounded border border-zinc-700">
                  <div className="text-sm">{item.content || JSON.stringify(item).slice(0, 200)}</div>
                </div>
              ))}
            </div>
          </div>
