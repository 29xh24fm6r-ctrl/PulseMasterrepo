// Strategy Domain Autonomy Policies
 ...
 * Detects when life arcs have stalled
 */
 ...
 * Ensures daily work aligns with quarterly focus
 */
const quarterlyAlignmentPolicy: AutonomyPolicy = {
  ...
  evaluate: (ctx: PulseCortexContext): AutonomyAction[] => {
    ...
    // Check if work queue items relate to Big 3
    ...
    const alignmentRatio = ctx.domains.work.queue.length > 0
...
registerAutonomyPolicy(stalledArcPolicy);
registerAutonomyPolicy(quarterlyAlignmentPolicy);
