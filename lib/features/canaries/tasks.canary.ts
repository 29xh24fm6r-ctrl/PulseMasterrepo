// lib/features/canaries/tasks.canary.ts
import "server-only";
...
export const tasksCanary: CanaryFn = async (ctx: CanaryContext): Promise<CanaryResult> => {
  const checks: CanaryResult["checks"] = [];
  const featureId = "tasks";
  ...
  try {
    ...
    checks.push({
      ...
      ok: apiOk,
      details: apiOk ? "GET /api/tasks responds" : `HTTP ${response.status}`,
      fixHint: apiOk ? undefined : hintApiDown("/api/tasks"),
    });
  } catch (err: any) {
    checks.push({
      id: "api_reachable",
  ...
  try {
    ...
    checks.push({
      ...
      ok: tableOk,
      details: tableOk ? "tasks table exists and is queryable" : error?.message,
      fixHint: tableOk ? undefined : isMissingTable ? hintMissingTable("tasks") : hintRlsDenied("tasks"),
    });
  } catch (err: any) {
    checks.push({
      id: "db_table_exists",
      label: "Database table accessible",
      ok: false,
      details: err?.message || "Failed to query tasks table",
      fixHint: hintMissingTable("tasks"),
    });
  }

  // Check 3: RLS sanity (can read own rows - via admin client, so this checks structure)
  try {
    const { data, error } = await supabaseAdmin.from("tasks").select("id, user_id").limit(1);
    const rlsOk = !error && (!data || data.length === 0 || data[0]?.user_id);
    checks.push({
      id: "rls_structure",
      label: "RLS structure valid",
      ok: rlsOk,
      details: rlsOk ? "tasks.user_id column present" : error?.message || "Missing user_id column",
      fixHint: rlsOk ? undefined : hintRlsDenied("tasks"),
    });
  } catch (err: any) {
    checks.push({
      id: "rls_structure",
