  ...
  try {
    ...
    return NextResponse.json({
      ...
      stats: {
        ...
        featureUsage,
        topUsers: topUsers?.map((u: any) => ({ oderId: u.user_id.slice(-8), email: u.email || "N/A", plan: u.plan, tokensUsed: u.usage_cents_this_month || 0 })),
        recentSignups: recentSignups?.map((u: any) => ({ oderId: u.user_id.slice(-8), email: u.email || "N/A", plan: u.plan, date: u.created_at })),
