...
export async function GET(req: Request) {
    ...
    const { data, error } = await supabaseAdmin.from("v_xp_totals").select("*").eq("user_id", user_id);
    if (error) return NextResponse.json({ ok: false, error: error.message }, { status: 500 });

    const { data: tracks, error: e2 } = await supabaseAdmin.from("belt_tracks").select("*").eq("is_active", true);
    if (e2) return NextResponse.json({ ok: false, error: e2.message }, { status: 500 });

    const totals: Record<string, number> = {};
    for (const r of data ?? []) totals[r.xp_type] = r.total;

    // For each track, pick the highest belt_level <= total
    const out: any[] = [];
    for (const t of tracks ?? []) {
        const xpTotal = totals[t.xp_type] ?? 0;
        const { data: levelRows, error: e3 } = await supabaseAdmin
            .from("belt_levels")
            .select("*")
            .eq("track_id", t.id)
