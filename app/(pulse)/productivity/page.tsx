...
export default function ProductivityDashboard() {
  ...
  const [nextFocusBlock, setNextFocusBlock] = useState("Good for moderate complexity tasks");
  ...
  // Safe fetch helper that never throws
  async function safeFetch<T>(url: string, defaultValue: T): Promise<T> {
    try {
      const res = await fetch(url);
      if (!res.ok) {
        if (process.env.NODE_ENV === "development") {
          console.warn(`[Productivity] API call failed: ${url} (${res.status})`);
        }
        return defaultValue;
      }
      ...
      // Silently handle network errors - never throw
      if (process.env.NODE_ENV === "development") {
        console.warn(`[Productivity] Network error for ${url}:`, error);
      }
      return defaultValue;
    }
  }
  ...
  async function fetchData() {
    // Fetch both APIs safely with defaults
    const [tasksData, emotionData] = await Promise.all([
      safeFetch<{ tasks?: Task[] }>("/api/tasks?status=pending,in_progress&limit=30", { tasks: [] }),
      safeFetch<{ states?: any[]; current?: any; today_trend?: any }>("/api/emotion?limit=3", { states: [], current: null, today_trend: null }),
    ]);

    // Process tasks with scoring
    const tasks = tasksData.tasks || [];
    const scored = tasks.map((t: Task) => {
      let score = 0;
    ...
    // Process emotion data for flow prediction
    // Handle both possible response structures: states array or current/today_trend
    let states = emotionData.states || [];
    if (!states.length && emotionData.current) {
      // If API returns current state, use it as a single state
      states = [emotionData.current];
    }
    
    const avgEnergy = states.length > 0 
      ? states.reduce((s: number, e: any) => s + (e.intensity || e.energy_level || 0.5), 0) / states.length 
      : 0.5;
    
    // Default to medium if no data
    if (avgEnergy > 0.7) {
      setFlowPrediction("high");
      setNextFocusBlock("Now is optimal for deep work");
    } else if (avgEnergy > 0.4) {
      setFlowPrediction("medium");
      setNextFocusBlock("Good for moderate complexity tasks");
    } else {
      setFlowPrediction("low");
      setNextFocusBlock("Consider lighter tasks or a break");
    }
  ...
  if (!isLoaded || loading) return <div className="flex items-center justify-center min-h-[400px]"><Loader2 className="w-8 h-8 animate-spin text-violet-500" /></div>;
