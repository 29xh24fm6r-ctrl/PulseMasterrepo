...
import { sendLowBalanceAlert, sendTokensDepletedEmail } from "./email/transactional";
...
async function hasAlertBeenSent(userId: string, threshold: number): Promise<boolean> {
  ...
  const { count } = await getSupabaseAdminRuntimeClient().from("usage_logs").select("*", { count: "exact", head: true }).eq("user_id_uuid", userId).eq("feature", `alert_${threshold}`).gte("created_at", startOfMonth.toISOString());
  return (count || 0) > 0;
}

async function markAlertSent(userId: string, threshold: number): Promise<void> {
  await getSupabaseAdminRuntimeClient().from("usage_logs").insert({ user_id_uuid: userId, owner_user_id_legacy: userId, feature: `alert_${threshold}`, tokens_used: 0, cost_cents: 0, model: "system", metadata: { type: "usage_alert", threshold } as any });
}

async function triggerAlert(userId: string, threshold: number, tokensRemaining: number, percentUsed: number): Promise<void> {
  const { data: profile } = await getSupabaseAdminRuntimeClient().from("user_profiles").select("email").eq("user_id_uuid", userId).single();
